...
for /d /r . %%d in (*.egg-info) do @if exist "%%d" rd /s /q "%%d"
for /r . %%f in (.coverage) do @if exist "%%f" del /q "%%f"
powershell -Command "Compress-Archive -Path * -DestinationPath mypkg.zip -CompressionLevel Optimal -Force"
powershell -Command "Remove-Item -Path mypkg.zip\*.git\* -Recurse -Force"
powershell -Command "Remove-Item -Path mypkg.zip\.vscode\* -Recurse -Force"
goto end
